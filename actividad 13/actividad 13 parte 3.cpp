...
void eliminarLibro(){
    char eliminar = misterio;
    cout << "eliminando el primer libro de misterio, segun el ordenS: ";


    Nodo *p = pInicio, *q = NULL;

    while(p != NULL && (p->dato).genero != eliminar){
        q = p;
        p = p->sig;
    }
    if(p == NULL){
        cout << "no hay libro de misterio" << endl;
        return;
    }
    if(q == NULL)
        pInicio = p->sig;
